...
[[ -z "$DEBUG" ]] || sed 's/^#debug/debug/g' /etc/caddyfile > /tmp/caddyfile

[[ -z "$DEBUG" ]] || test -e /tmp/caddyfile && (while (true);do caddy run --config /tmp/caddyfile ;sleep 10;done ) &
[[ -z "$DEBUG" ]] && test -e /etc/caddyfile && (while (true);do caddy run --config /etc/caddyfile ;sleep 10;done ) &
